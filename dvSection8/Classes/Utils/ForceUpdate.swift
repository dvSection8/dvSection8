...
public struct ForceUpdate {
    ...
    */
     ...
     */
    fileprivate func requests(_ id: Any) -> APIRequests {
        let url = URL(string: iTunesAffiliates.Lookup.ph.rawValue + "\(id)")
        let headers = ["content-Type": contentType()]
        return APIRequests(.post, url: url, parameters: nil, headers: headers)
    }
    ...
    */
    ...
    public func checkAppVersion(_ update: @escaping () -> ()) {
        getAppStoreVersion { (value) in
            ...
            let deviceAppVersion = DeviceManager().appVersion
